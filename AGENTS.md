...
- Repo layout: `arabic/` (runtime), `reference/` (canonical library), `docs/` (product, planning, engineering, supported), `scripts/` (validation); `website/` Next.js App Router marketing site on `main` (PR #46; G13–G17 green; **G18 Vercel preview** pending before `v1.1.0` tag); Masri copy in `website/content/` (8 routes + `footer.md`); install docs in README + `docs/supported/` until site ships; v1.1 Stitch specs and dated execution plans in `docs/planning/`; G14 parity via `scripts/validate-website-install.sh` (in `npm run validate`); website golden tests `tests/golden/g13-g18-website.md`; in `docs/` markdown use plain `/route` notation for website paths (not `[text](/route)` — `validate-docs.sh` treats absolute links as filesystem paths); lightweight tool SVGs in `public/assets/` (full brand packs e.g. `cline-brand-assets/` gitignored).
